...
module.exports = class PayablesModule extends OraclePage {
  ...
  async readScreen() {
    ...
    const hasExpenseDrop = await this.page.evaluate(() =>
      !!document.querySelector('[id$=":socExpenseWA::drop"]'));
        ...
        if (popup.title === 'Add Expense Items') {
          ...
          popup.items = Array.from(rootEl.querySelector('div[id$="::db"]').children).map(x => {
            ...
            return {
              ...
              date: x.querySelector('.xqu').textContent,
      ...
      const {fields, note} = await this.page.evaluate(() => {
        ...
        const fields = Array.from(wrapper
          ...
          .map(xt1 => {
            ...
            const lastAttachment = xt1.querySelector('a[id$=":lastAttachedFile"]');
            if (attachmentsBtn) {
            ...
            if (xt1.id.endsWith('Label')) {
              data.type = 'label';
              data.value = xt1.querySelectorAll('td')[1].innerText.trim();

              const button = xt1.querySelector('button[id]');
              if (button) {
  ...
  async readItems() {
    ...
    const items = await this.page.evaluate(() => {
      ...
      return Array.from(document.querySelector('div[id$="::db"]').children).map(x => {
        ...
        const x2ua = Array.from(x.querySelectorAll('.x2ua'));
        if (x2ua.length === 3) {
          x2ua.splice(2, 0, {textContent: ''});
        }
        return {
          selector: expButton && ('#'+expButton.id.replace(/:/g,'\\:')),
          category: expButton && expButton.textContent,
          location: x.querySelector('.x2ub').textContent,
          date: x2ua[0].textContent,
          amount: x.querySelector('.AppsBoldFont').textContent,
          currency: x2ua[1].textContent,
          deleteSelector: '#'+x.querySelector('a[title="Delete"]').id.replace(/:/g,'\\:'),
          merchant: x2ua[2].textContent,
          description: x2ua[3].textContent,
        };
      });
    });
